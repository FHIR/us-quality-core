...
For more information about model information for usage in CQL, refer to the [ModelInfo](modelinfo.html) page of this guide.
...
FHIR resources. By defining extensions in a uniform way as part of the base specification, FHIR enables extension-based functionality to be introduced through the use of profiles and implementation guidance. US Quality Core includes several extensions related to quality improvement applications.
...
the `Patient.deceased` element can be specified as a `Boolean` or as a `DateTime`. Since CQL also supports choice types, these are manifest directly as Choice Types within the ModelInfo.
