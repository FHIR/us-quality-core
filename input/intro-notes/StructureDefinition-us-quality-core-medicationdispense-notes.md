...
1. **SHALL** support searching for records associated with a patient using the **[`patient`](SearchParameter-us-quality-core-medicationdispense-patient.html)** search parameter:

   `GET [base]/MedicationDispense?patient={Patient/}[id]`

   Example:

     1. GET [base]/MedicationDispense?patient=Patient/1137192
   ...
   *Implementation Notes:* Fetches a Bundle of MedicationDispense resources for the specified patient. ([how to search by reference]({{site.data.fhir.path}}search.html#reference))

2. **SHALL** support searching using the combination of the **[`patient`](SearchParameter-us-quality-core-medicationdispense-patient.html)** and **[`status`](SearchParameter-us-quality-core-medicationdispense-status.html)** search parameters:

   - including support for *OR* search on `status` (e.g. `status={system|}[code],{system|}[code],...`)

   `GET [base]/MedicationDispense?patient={Patient/}[id]&status={system|}[code]`

   Example:

     1. GET [base]/MedicationDispense?patient=Patient/1137192&status=completed
   ...
   *Implementation Notes:* Fetches a Bundle of MedicationDispense resources filtered by status. ([how to search by reference]({{site.data.fhir.path}}search.html#reference) and [how to search by token]({{site.data.fhir.path}}search.html#token))
